...
public class SystemExternalizedProperties implements ExternalizedProperties {
     ...
     */
    public SystemExternalizedProperties(Resolver rootResolver, Converter<?> rootConverter, VariableExpander variableExpander, InvocationHandlerFactory invocationHandlerFactory) {
    ...
    private static void throwIfVoidReturnType(Method proxyMethod) {
        ...
        if (returnType.equals(Void.TYPE) || returnType.equals(Void.class)) {
            throw new IllegalArgumentException("Proxy methods must not have void return type.");
        }
    }

    private static void throwIfExclusiveAnnotationsAreFound(Method proxyMethod) {
        Annotation[] d = proxyMethod.getAnnotations();
        String[] annotationNames = Arrays.stream(d).map(Annotation::annotationType).filter(SystemExternalizedProperties::isExclusiveAnnotation).map(Class::getName).toArray(String[]::new);
        if (annotationNames.length > 1) {
            throw new IllegalArgumentException("Exclusive annotations detected: " + Arrays.toString(annotationNames) + ". These annotations may only be use exclusive of each other.");
        }
    }
    ...
    private static void throwIfInvalidResolverFacadeMethodSignature(Method proxyMethod) {
        ...
        if (parameterTypes.length == 0 || parameterTypes.length > 2) {
            throw new IllegalArgumentException(String.format("Resolver facades must either accept a single 'property name' parameter (%s) " + "or two parameters - the 'property name' (%s) and the 'target type' (%s).", String.class.getName(), String.class.getName(), SUPPORTED_TARGET_TYPES));
        }
        Class<?> parameterType = parameterTypes[0];
        if (!String.class.equals(parameterType)) {
            throw new IllegalArgumentException(String.format("Resolver facades must accept 'property name' (%s) as first parameter.", String.class.getName()));
        }
        if (parameterTypes.length == 2) {
            Class<?> secondParameterType = parameterTypes[1];
            if (!SUPPORTED_TARGET_TYPES.contains(secondParameterType)) {
                throw new IllegalArgumentException(String.format("Resolver facades must accept 'target type' (%s)" + "as second parameter.", SUPPORTED_TARGET_TYPES));
            }
        }
    }
    ...
    private static void throwIfInvalidConverterFacadeMethodSignature(Method proxyMethod) {
        ...
        Class<?>[] parameterTypes = proxyMethod.getParameterTypes();
        if (parameterTypes.length == 0 || parameterTypes.length > 2) {
            throw new IllegalArgumentException(String.format("Converter facades must either accept a single 'value to convert' parameter (%s) " + "or two parameters - the 'value to convert' (%s) and the 'target type' (%s).", String.class.getName(), String.class.getName(), SUPPORTED_TARGET_TYPES));
        }
        Class<?> firstParamType = parameterTypes[0];
        if (!String.class.equals(firstParamType)) {
            throw new IllegalArgumentException(String.format("Converter facades must accept 'value to convert' (%s) as first parameter.", String.class.getName()));
        }
        if (parameterTypes.length == 2) {
            Class<?> secondParamType = parameterTypes[1];
            if (!SUPPORTED_TARGET_TYPES.contains(secondParamType)) {
                throw new IllegalArgumentException(String.format("Converter facades must accept 'target type' (%s)" + "as second parameter.", SUPPORTED_TARGET_TYPES));
            }
        }
    }
    ...
    private static void throwIfInvalidVariableExpanderFacadeMethodSignature(Method proxyMethod) {
        ...
        if (parameterTypes.length != 1) {
            throw new IllegalArgumentException("Variable expander facades must have a single parameter.");
        }
        Class<?> parameterType = parameterTypes[0];
        if (!String.class.equals(parameterType)) {
            throw new IllegalArgumentException("Variable expander facades must have a single String parameter.");
        }
        if (!String.class.equals(proxyMethod.getReturnType())) {
            throw new IllegalArgumentException("Variable expander facades must have a String return type.");
        }
    }
    ...
    private static boolean isExclusiveAnnotation(Class<?> annotationType) {
        ...
        return ExternalizedProperty.class.equals(annotationType) || ResolverFacade.class.equals(annotationType) || ConverterFacade.class.equals(annotationType) || VariableExpanderFacade.class.equals(annotationType);
    }

    private static Set<Class<?>> supportedTargetTypes() {
        Set<Class<?>> weakSet = Collections.newSetFromMap(new WeakHashMap<>());
        weakSet.addAll(Arrays.asList(TypeReference.class, Class.class, Type.class));
