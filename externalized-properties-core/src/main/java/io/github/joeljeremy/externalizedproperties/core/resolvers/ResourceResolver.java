...
public class ResourceResolver extends MapResolver {
    ...
    private static Map<String, String> readMapFromUrl(URL url, ResourceReader reader) {
        ...
        try {
            ...
            throw new ExternalizedPropertiesException(String.format("An exception occurred while trying to read resource: %s", url.toString()), e);
        }
        return result;
    }
     ...
     */
    private static void buildFlattenedMap(Map<String, String> result, Map<String, Object> source, @Nullable String parentPath) {
    ...
    private static void buildFlattenedCollection(Map<String, String> result, Collection<Object> source, String key) {
        if (source.isEmpty()) {
            result.put(key, "");
            return;
        }
    ...
    public static class PropertiesReader implements ResourceReader {
        ...
        private static Map<String, Object> toMap(Properties properties) {
            return properties.entrySet().stream().filter(e -> e.getKey() instanceof String).collect(Collectors.toMap(e -> (String) e.getKey(), Map.Entry::getValue));
        }
    }
}
