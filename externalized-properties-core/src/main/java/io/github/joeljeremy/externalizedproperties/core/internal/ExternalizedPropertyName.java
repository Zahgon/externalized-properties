...
public class ExternalizedPropertyName {
    ...
    private static String determineNameFromInvocationArgs(InvocationArguments invocationArgs) {
        ...
        Optional<String> nameFromArguments = invocationArgs.get(0).filter(String.class::isInstance).map(String.class::cast).filter(a -> !a.trim().isEmpty());
        if (!nameFromArguments.isPresent()) {
            throw new IllegalArgumentException("Please provide the externalized property name via method arguments. " + "Only String values are allowed. Null or blank values are not allowed.");
        }
