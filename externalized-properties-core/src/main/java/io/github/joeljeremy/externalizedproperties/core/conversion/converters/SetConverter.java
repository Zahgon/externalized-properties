...
public class SetConverter implements Converter<Set<?>> {
     ...
     */
    @Override
    public ConversionResult<Set<?>> convert(InvocationContext context, String valueToConvert, Type targetType) {
        throw new UnsupportedOperationException("STUB: not implemented");
    }
    ...
    private Set<Object> newSet(int capacity) {
        ...
        if (set == null || !set.isEmpty()) {
            throw new IllegalStateException("Set factory implementation must not return null or a populated set.");
        }
        return set;
    }
